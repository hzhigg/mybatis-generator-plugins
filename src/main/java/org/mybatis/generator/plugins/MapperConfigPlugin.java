...
public class MapperConfigPlugin extends PluginAdapter {
    ...
    public List<GeneratedXmlFile> contextGenerateAdditionalXmlFiles() {
        ...
        //在XmlConstants里面还有很多常量，比如MYBATIS3_MAPPER_SYSTEM_ID和MYBATIS3_MAPPER_PUBLIC_ID
        Document document = new Document(
                XmlConstants.MYBATIS3_MAPPER_CONFIG_PUBLIC_ID,
                XmlConstants.MYBATIS3_MAPPER_CONFIG_SYSTEM_ID);

        //接着创建根目录，<configuration>，和JavaDOM基本一样
        XmlElement root = new XmlElement("configuration"); //$NON-NLS-1$
        document.setRootElement(root);

        //添加注释，因为这里做的有点不太规范，最好还是使用MBG提供的context.getCommentGenerator的addComment(XmlElement xmlElement)方法来统一生成注释
